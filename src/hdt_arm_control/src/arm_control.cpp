...
{ //Setting ROS Handle
	...
	ros::NodeHandle nh;
	ros::NodeHandle pnh("~");
	endpoint_state_pub = nh.advertise<geometry_msgs::Pose>("status", 1);
	...
	pnh.param<bool>("use_unity", use_unity, false);
	...
	if (use_unity)
	{
		unity_sub = nh.subscribe(UNITY_TOPIC, 100, &ArmControl::UnityCb, this);
		unity_fingers_sub = nh.subscribe(UNITY_FINGERS_TOPIC, 100, &ArmControl::UnityFingersCb, this);
	}
...
void ArmControl::JoyControlArmCb(void)
{
	...
	{
		endpoint_state_pub.publish(endpoint_pose);
...
void ArmControl::JoyControlHandCb(void)
{
	...
	else
	{
		...
		{
		// Leap Motion && Unity VR
		// update hand command
		hand_group.joint_cmd_msg.position[0] =  theta_index;	// open and close of index finger
		hand_group.joint_cmd_msg.position[1] =  theta_middle;	// open and close of middle finger
		hand_group.joint_cmd_msg.position[2] =  phi_thumb;		// rotation of the thumb
		hand_group.joint_cmd_msg.position[3] =  theta_thumb;    // open and close of the thumb
		}
		//Making sure we dont close and rotate the fingers at the same time
		if (joy_msg.axes[6] != 0)
			joy_msg.axes[7] = 0;
...
bool ArmControl::CollisionCb(const planning_scene::PlanningScene *planning_scene, robot_state::RobotState *state, const robot_state::JointModelGroup *group, const double *joint_group_variable_values)
{
	...
	if (res.collision)
	{
		...
		for (int i = 0; i < colliding_links.size(); i++)
			ROS_ERROR("%s", colliding_links[i].c_str());
		return false;
	}
 ...
 *----------------------------------------------------------------------------*/
void ArmControl::UnityCb(const geometry_msgs::Pose &msg)
{
	endpoint_pose.position = msg.position;
	endpoint_pose.orientation = msg.orientation;
}

void ArmControl::UnityFingersCb(const sensor_msgs::ChannelFloat32 &msg)
{
	phi_thumb = msg.values[0];
	theta_thumb = msg.values[1];
	theta_index = msg.values[2];
	theta_middle = msg.values[3];

	// ROS_INFO("[%f,%f,%f,%f]",msg.values[0]*180/PI,msg.values[1]*180/PI,msg.values[2]*180/PI,msg.values[3]*180/PI);
}
